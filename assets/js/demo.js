...
$(document).ready(function () {
  ...
  var showColorsForImage = function($image, $imageSection ) {
    ...
    var paletteRGB               = colorThief.getPalette(image);
    var arrayLength              = paletteRGB.length;
    var palette                  = [];
    ...
    for (var i = 0; i < arrayLength; i++) {
      var h = {};
      var color = paletteRGB[i];
      var hex = rgbToHex(color[0], color[1], color[2]);
      var name = ntc.name(hex);

      h[0] = hex; /* Color in Hex*/ 
      h[1] = name[1]; /* Color name */ 
      h[2] = name[2]; /* Is exact match? */ 
      palette.push(h);
    }
